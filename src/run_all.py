...
def main():
    ...
    main_files = list(run_dir.glob("main_*.py"))
    
    tf_files = list(run_dir.glob("main_cifar*.py")) + list(run_dir.glob("main_*cnn*.py"))
    other_files = [f for f in main_files if f not in tf_files]
    
    for main_file in other_files:
